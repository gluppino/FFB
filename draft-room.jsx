...
/* ---------- Scoring engine, built to the league's exact rules ---------- */
// Per-unit rules taken directly from the league scoring screenshots.
// Threshold bonuses (40+/50+ yd TD, 100-199/200+ yd games, 300-399/400+ pass games)
// can't be computed from season totals alone — they need game logs. They're
// estimated below from season averages using a simple probability curve, and are
// flagged in the UI as approximate. Everything else (yards, TDs, INTs, receptions,
// fumbles, kicking, DST) is exact arithmetic against the season stat projection.
const GAMES = 17;

function bigPlayBonus(tds) {
  // Assume ~16% of TDs go for 40+ yds and ~7% go for 50+ (stacking), a reasonable
  // league-average big-play rate. Rough by design — edit a player's TDs to adjust.
...
// Season stat projections (rounded, estimated from expected role/usage — not sourced).
// Shape: RB/WR -> [rushYd,rushTD,rec,recYd,recTD,fuml] | QB -> [passYd,passTD,int,rushYd,rushTD,fuml] | TE -> [rec,recYd,recTD,fuml]
const RB_WR_STATS = {
